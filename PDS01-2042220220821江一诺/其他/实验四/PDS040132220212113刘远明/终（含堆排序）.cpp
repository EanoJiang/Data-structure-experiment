...
//快速排序
int Division(RecType R[],int left,int right)  //分割；
{
	RecType base=R[left];  //基准元素；
	while(left<right)
	{
        //先从后向前找，再从前向后找
		while(left<right&&R[right].key>=base.key)
			right--;   //从右向左找第一个比基准值小的元素；
		R[left]=R[right];
		while(left<right&&R[left].key<base.key)
...
void QuickSort(RecType R[],int left,int right)
{
	if(left<right)
	{
		int i=Division(R,left,right);  //分割；
		QuickSort(R,left,i-1);  //将两部分分别排序；
...
//堆排序
void HeapAdjust(RecType R[],int s,int m)//一次筛选的过程
{
    int i;
    RecType rc;
	rc=R[s];
    for(i=2*s;i<=m;i=i*2)//通过循环沿较大的孩子结点向下筛选
    {
        if(i<m&&R[i].key<R[i+1].key) i++;//j为较大的记录的下标
        if(rc.key>R[i].key) break;
        R[s]=R[i];s=i;
    }
    R[s]=rc;//插入
}
void HeapSort(RecType R[],int n)
{
    ...
    for(i=n;i>0;i--)
    {
        temp=R[0];
        R[0]=R[i];
        R[i]=temp;//将堆顶记录与未排序的最后一个记录交换
        HeapAdjust(R,0,i-1);//重新调整为顶堆
    }
}
